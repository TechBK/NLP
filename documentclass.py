...
class DocumentClass(Document):
    ...
    # Probalitity of word given the class self: p(tk|c)
    def Probability(self,word):
        """ returns the probabilty of the word "word" given the class "self" """
        voc_len = Document._vocabulary.len()    #Sum of Word in BagVocalury
        SumN = 0

        for i in range(voc_len):
            SumN = DocumentClass._vocabulary.WordFreq(word)

        N = self._words_and_freq.WordFreq(word)  # = frequency of a "word"
        erg = 1 + N
        erg /= voc_len + SumN
        return erg
